...
describe("SQLite Database Service", () => {
  ...
  describe("Transaction Support", () => {
    test("should support transactions", async () => {
      ...
      db.transaction(() => {
        db.query("INSERT INTO projects (id, name) VALUES (?, ?)", [
          projectId,
          "Transaction Test",
        ]);
      });
  ...
  describe("Foreign Key Constraints", () => {
    ...
    test("should cascade delete when parent is deleted", async () => {
      ...
      const noteResult = await db.query("SELECT * FROM notes WHERE id = ?", [
        noteId,
      ]);
  ...
  describe("Error Handling", () => {
    test("should throw error for invalid SQL", async () => {
      await expect(db.query("INVALID SQL STATEMENT")).rejects.toThrow();
    });
