  ...
  try {
    ...
    for (const table of tables) {
      try {
        ...
        // Table might not exist or database might be closed, ignore
      }
    }
  } catch (error) {
...
export async function createTestProject(
  ...
  const result = await db.query("SELECT * FROM projects WHERE id = ?", [
    projectId,
  ]);
...
export async function createTestMilestone(
  ...
  const result = await db.query("SELECT * FROM milestones WHERE id = ?", [
    milestoneId,
  ]);
  return result.rows[0];
}
...
export async function createTestPreference(
  ...
  const result = await db.query("SELECT * FROM user_preferences WHERE id = ?", [
    prefId,
  ]);
  return result.rows[0];
}
