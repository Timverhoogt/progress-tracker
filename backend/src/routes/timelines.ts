...
router.put('/milestones/:id', async (req, res) => {
  try {
    ...
    const result = await db.query(`UPDATE milestones SET ${updates.join(', ')} WHERE id = ?`, values);
    if (result.rowsAffected === 0) return res.status(404).json({ error: 'Milestone not found' });
    const fetch = await db.query('SELECT * FROM milestones WHERE id = ?', [id]);
...
router.post('/estimate', async (req, res) => {
  try {
    ...
    const projectRes = await db.query('SELECT name, description FROM projects WHERE id = ?', [project_id]);
