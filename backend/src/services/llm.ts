...
class LLMService {
  ...
  private async callLLM(messages: any[]): Promise<LLMResponse> {
    try {
      ...
      return {
        success: false,
        error:
          error.response?.data?.error?.message || "LLM service unavailable",
      };
    }
  }
  ...
  private async getPersonalityDirectives(): Promise<string> {
    ...
    const detailLevel = await settingsService.get<string>(
      "llm_detail_level",
      "balanced"
    );
    const language = await settingsService.get<string>("llm_language", "auto");
    const preset = await settingsService.get<string>(
      "llm_preset_template",
      "default"
    );
    ...
    switch (tone) {
      case "friendly":
        directives.push(
          "Tone: Use a warm, approachable, and supportive communication style."
        );
        break;
      case "formal":
        directives.push(
          "Tone: Maintain strict formality and professional business language."
        );
        break;
      case "concise":
        directives.push(
          "Tone: Be direct, brief, and eliminate unnecessary words."
        );
        break;
      case "enthusiastic":
        directives.push(
          "Tone: Show energy, positivity, and excitement about the work."
        );
        break;
      case "analytical":
      ...
      case "creative":
        directives.push(
          "Tone: Encourage innovative thinking and creative solutions."
        );
        break;
      case "professional":
      default:
        directives.push(
          "Tone: Maintain professional, clear, and business-appropriate language."
        );
    }
    ...
    switch (detailLevel) {
      case "brief":
        directives.push(
          "Detail level: Provide concise, essential information only."
        );
        break;
      case "detailed":
        directives.push(
          "Detail level: Include comprehensive explanations and thorough analysis."
        );
        break;
      case "comprehensive":
        directives.push(
          "Detail level: Provide exhaustive coverage with examples, context, and implications."
        );
        break;
      case "balanced":
      default:
        directives.push(
          "Detail level: Provide adequate detail without being overly verbose."
        );
    }
    ...
    if (language && language !== "auto") {
      ...
      if (languageMap[language]) {
        directives.push(
          `Language: Respond primarily in ${languageMap[language]} unless specifically asked otherwise.`
        );
      }
    }
    ...
    switch (preset) {
      case "consultant":
        directives.push(
          "Context: You are advising as an experienced management consultant focused on process improvement and strategic recommendations."
        );
        break;
      case "analyst":
        directives.push(
          "Context: You are providing analysis as a business analyst, emphasizing data-driven insights and systematic evaluation."
        );
        break;
      case "manager":
        directives.push(
          "Context: You are communicating as a project manager, focusing on execution, timelines, and stakeholder coordination."
        );
        break;
      case "technical":
        directives.push(
          "Context: You are providing technical expertise, emphasizing implementation details and technical best practices."
        );
        break;
      case "creative":
        directives.push(
          "Context: You are brainstorming as a creative strategist, encouraging innovative approaches and out-of-the-box thinking."
        );
        break;
      case "default":
      default:
        directives.push(
          "Context: You are Tim Verhoogt's AI assistant, helping with continuous improvement work at Evos Amsterdam."
        );
    }
...
- Ensure recommendations align with operational excellence principles and Evos Amsterdam's business objectives`;
  ...
  private async getGlobalPreamble(): Promise<string> {
    const mode = await settingsService.get<string>(
      "llm_system_prompt_mode",
      "generated"
    );

    if (mode === "custom") {
      const customPrompt = await settingsService.get<string>(
        "llm_system_prompt",
        ""
      );
      return customPrompt || "";
  ...
  async enhanceNote({
    content,
    projectContext,
  }: EnhanceNoteRequest): Promise<LLMResponse> {
...
Task: Transform a casual diary-style project note into a professional, structured version suitable for reporting and documentation.

${personalityDirectives}

PURPOSE:
The original casual note is preserved separately. Your enhanced_content should be a professional, report-ready version that:
- Maintains the core information and meaning from the original
- Uses professional business language appropriate for stakeholder communication
- Expands on the content with relevant context from the project
- Provides clear structure and completeness for documentation purposes

CRITICAL RULES:
1. PROFESSIONAL TONE: Transform casual language into clear, professional business communication suitable for reports and stakeholder updates
2. AUGMENT WITH CONTEXT: Integrate relevant project details and connections to enrich the content - make it more complete and informative
3. EXPAND THOUGHTFULLY: Make it longer and more detailed than the original, adding professional context and implications while staying grounded in the actual content
4. NO AWKWARD PLACEHOLDERS: Don't add [date] or similar placeholders unless you can infer specific timing from context
5. MEANINGFUL ENHANCEMENT: Transform "he was very positive" into something like "received positive validation and constructive feedback" - professional but accurate to the sentiment
6. STRUCTURED & CLEAR: Organize information logically with proper business writing structure

Enhancement Guidelines:
- Transform casual observations into professional statements
- Add relevant business context from the project details provided
- Expand brief mentions into fuller explanations that would be appropriate in a progress report
- Connect ideas to project objectives and strategic value
- Use professional terminology while avoiding unnecessary jargon
- Create a version someone could use directly in a status update or report
- Extract comprehensive structured data that enables future analysis and insights
...
${content}`,
  ...
  async generateTodos({
    ...
    projectDescription,
    recentNotes,
  }: GenerateTodosRequest): Promise<LLMResponse> {
    const preamble = await this.getGlobalPreamble();
    const personalityDirectives = await this.getPersonalityDirectives();
    const messages = [
      ...(preamble ? [{ role: "system", content: preamble }] : []),
...
[
  ...
  async generateReport({
    ...
    reportType,
    recipient,
  }: GenerateReportRequest): Promise<LLMResponse> {
    const reportPrompts = {
      status:
        "Generate a professional status report for management, highlighting progress, challenges, and next steps.",
      summary:
        "Create a comprehensive project summary suitable for documentation or handover.",
      stakeholder:
        "Prepare a stakeholder-friendly report focusing on business impact and outcomes.",
    };
...
- Reflect Evos Amsterdam's focus on operational excellence`,
  ...
  async estimateTimeline({
    ...
    datedTodos,
    milestones,
  }: EstimateTimelineRequest): Promise<LLMResponse> {
    const preamble = await this.getGlobalPreamble();
    const personalityDirectives = await this.getPersonalityDirectives();
    const messages = [
      ...(preamble ? [{ role: "system", content: preamble }] : []),
...
${
  datedTodos
    .map(
      (t) => `- ${t.title} | ${t.due_date || "n/a"} | ${t.status || "pending"}`
    )
    .join("\n") || "none"
}
...
${
  (milestones || [])
    .map(
      (m) =>
        `- ${m.title} | ${m.target_date || "n/a"} | ${m.status || "planned"}`
    )
    .join("\n") || "none"
}`,
      },
...
- Managing ${projectContext.totalProjects || 0} projects (${
        projectContext.activeProjects || 0
      } active)
- Overall task completion rate: ${projectContext.completionRate || 0}%
- Current workload: ${projectContext.pendingTodos || 0} pending tasks, ${
        projectContext.completedTodos || 0
      } completed
- High priority items: ${
        projectContext.highPriorityPending || 0
      ...
      if (
        ...
        projectContext.projectSummaries.length > 0
      ) {
        const projectLines = projectContext.projectSummaries
          .map(
            (p: any) =>
      ...
      let recentWork = "";
      if (
        projectContext.recentWorkNotes &&
        projectContext.recentWorkNotes.length > 0
      ) {
        const workLines = projectContext.recentWorkNotes
          .slice(0, 3)
          .join("\n- ");
      ...
      contextInfo =
        portfolioSummary + projectInsights + recentWork + currentFocus;
    }
