...
class BackupService {
  ...
  static async createBackup(options: BackupOptions = {}): Promise<string> {
    ...
    const backupPath = options.backupPath || path.join(backupDir, backupFileName);
  ...
  static async restoreBackup(backupPath: string): Promise<void> {
    ...
    try {
      const db = getDatabase();
      const currentDbPath = path.join(process.cwd(), 'data', 'progress_tracker.db');
      ...
      console.log('⚠️  Please restart the application to reconnect to the restored database.');
    } catch (error) {
